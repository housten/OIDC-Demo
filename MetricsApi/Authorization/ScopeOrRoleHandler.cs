...
public class ScopeOrRoleHandler : AuthorizationHandler<ScopeOrRoleRequirement>
{
    protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, ScopeOrRoleRequirement requirement)
    {
        ...
        if (scopeClaim != null && scopeClaim.Value.Split(' ', StringSplitOptions.RemoveEmptyEntries).Contains(requirement.Scope))
        {
        ...
        if (roleClaim != null && roleClaim.Value.Split(' ', StringSplitOptions.RemoveEmptyEntries).Contains(requirement.Role))
        {
            context.Succeed(requirement);
            return Task.CompletedTask;
        }
